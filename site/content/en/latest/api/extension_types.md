...
| `disableXForwardedFor` | _boolean_ |  false  |  | DisableXForwardedFor configures Envoy Proxy to stop appending the downstream address<br />to the X-Forwarded-For header.<br />This only disables the automatic append/add behavior. It does not remove or sanitize<br />an incoming X-Forwarded-For header. |
